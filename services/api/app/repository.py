...
from app.queueing import enqueue_job_record
...
def export_dd_report_artifact(dd_report_id: str, format: str) -> tuple[bytes | str, str, str] | None:
    ...
    if report is None:
        return None
    if format == "memo":
        filename = f"{dd_report_id}-memo.md"
        put_artifact_object(
            f"{report.project_id}/dd-reports",
            filename,
            report.memo_markdown.encode("utf-8"),
        )
        return report.memo_markdown, "text/markdown", filename
    if format == "exceptions":
        content = "\n".join(f"- {item}" for item in report.exceptions_list)
        filename = f"{dd_report_id}-exceptions.txt"
        put_artifact_object(
            f"{report.project_id}/dd-reports",
            filename,
...
def export_query_run_csv(query_run_id: str) -> str | None:
    ...
    put_artifact_object(
        f"{query_run.project_id}/queries",
        f"{query_run_id}.csv",
...
def export_workflow_run_xlsx(workflow_run_id: str) -> tuple[bytes, str, str] | None:
    ...
    filename = f"{workflow_run_id}-workflow-results.xlsx"
    put_artifact_object(
        f"{workflow_run.project_id}/workflow-runs",
        filename,
        content,
    )
    return (
...
def export_dd_report_docx(report: DdReportRecord) -> tuple[bytes, str, str]:
    ...
    content = output.getvalue()
    filename = f"{report.id}-memo.docx"
    put_artifact_object(
        f"{report.project_id}/dd-reports",
        filename,
        content,
...
def create_review_run_export_record(
    ...
    put_artifact_object(
        f"{review_run.project_id}/review-runs",
        f"{export_id}.md",
    ...
    try:
        with get_connection() as connection:
            for filename, content in files:
                ...
                temp_file_path = temporary_source_file(stored_name, content)
                stored_object = put_source_object(workspace_id, stored_name, content)
                stored_locations.append(stored_object.location)
                ...
                insert_document(
                    ...
                    file_path=stored_object.location,
                    file_hash=file_hash,
                )
...
def create_job_record(
    ...
    if status == JobStatus.QUEUED:
        rq_job_id = enqueue_job_record(job_id, job_type)
        if rq_job_id:
            metadata["rq_job_id"] = rq_job_id
            connection.execute(
                "UPDATE job_records SET metadata_json = ? WHERE id = ?",
    ...
    with get_connection() as connection:
        ...
        if row["status"] == JobStatus.QUEUED:
            connection.execute(
                ...
                WHERE id = ?
                """,
                (JobStatus.RUNNING, now_timestamp(), worker_name, job_id),
            )
...
def delete_document_record(document_id: str) -> bool:
    ...
    with get_connection() as connection:
        row = connection.execute(
            "SELECT workspace_id, file_path FROM documents WHERE id = ?",
            (document_id,),
        ).fetchone()
        if row is None:
            return False
        ...
        version_rows = connection.execute(
            "SELECT file_path FROM document_versions WHERE source_document_id = ?",
            (document_id,),
        ).fetchall()
        ...
        for version_row in version_rows:
            if version_row["file_path"] and version_row["file_path"] != row["file_path"]:
                delete_object(version_row["file_path"])
        connection.execute(
            "UPDATE workspaces SET last_updated = ? WHERE id = ?",
            (now_timestamp(), row["workspace_id"]),
        )
        return True
...
def delete_project_record(project_id: str) -> bool:
    ...
    with get_connection() as connection:
        ...
        connection.execute("DELETE FROM workspaces WHERE id = ?", (project_row["workspace_id"],))
        for object_row in object_rows:
            if object_row["file_path"]:
                delete_object(object_row["file_path"])
        delete_artifact_prefix(project_id)
        return True
